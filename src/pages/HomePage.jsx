...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (!raw) return;
    try {
      const { sessionId, code, ts } = JSON.parse(raw);
      if (Date.now() - ts > 2 * 60 * 60 * 1000) { localStorage.removeItem('lastSession'); return; }
      get(ref(db, `sessions/${sessionId}/meta/status`)).then((snap) => {
        const status = snap.val();
        if (status && status !== 'ended' && status !== 'closed') {
          setLastSession({ sessionId, code, status });
        } else {
  ...
  const statusLabel = lastSession?.status === 'waiting' ? 'In der Lobby'
    : lastSession?.status === 'hiding' ? 'Spiel läuft (Verstecken)'
    : 'Spiel läuft';
      ...
      <div className="pt-safe relative flex flex-col h-full px-4 py-6 max-w-sm mx-auto w-full overflow-y-auto" style={{ zIndex: 1 }}>
        ...
        <div className="flex items-center justify-between mb-8" style={{ animation: 'fadeIn 0.4s ease both' }}>
          <div className="flex items-center gap-3">
            <img src="/logo.png" className="w-10 h-10 rounded-xl object-cover" alt="LocationHunt" style={{ animation: 'float 4s ease-in-out infinite' }} />
            <span className="text-white font-bold text-lg" style={{ textShadow: '0 1px 6px rgba(0,0,0,0.6)' }}>LocationHunt</span>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/settings')} className="cursor-pointer" style={{ flexShrink: 0 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', border: `2px solid ${color}`, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(22,27,34,0.8)', backdropFilter: 'blur(8px)' }}>
                {profile?.photoBase64
            ...
            <button onClick={handleLogout} className="text-sm cursor-pointer transition-colors" style={{ color: 'rgba(255,255,255,0.7)', textShadow: '0 1px 4px rgba(0,0,0,0.5)' }}>
              Abmelden
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center gap-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            {lastSession && (
              <button
                onClick={() => navigate(`/lobby/${lastSession.sessionId}`)}
                className="rounded-2xl p-5 text-left cursor-pointer active:scale-95 transition-all"
                style={{
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-white font-bold text-base">Weiterspielen</h2>
                    <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.78rem' }}>
                      {statusLabel} · Code: <span style={{ color: '#22C55E', fontWeight: 700 }}>{lastSession.code}</span>
                    </p>
                  </div>
                </div>
              </button>
            ...
            >
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-3" style={{ background: 'rgba(59,130,246,0.2)' }}>
                <span className="text-game-blue text-2xl">＋</span>
              </div>
              <h2 className="text-white text-xl font-bold">Spiel erstellen</h2>
              <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.875rem', marginTop: '0.25rem' }}>Zone festlegen und Session starten</p>
            </button>
            ...
            <button
              ...
              onMouseLeave={(e) => e.currentTarget.style.borderColor = 'rgba(48,54,61,0.8)'}
            >
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-3" style={{ background: 'rgba(34,197,94,0.2)' }}>
                <span className="text-game-green text-2xl">→</span>
              </div>
              <h2 className="text-white text-xl font-bold">Spiel beitreten</h2>
              <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.875rem', marginTop: '0.25rem' }}>Mit Code oder QR-Code einer Session beitreten</p>
            </button>
          </div>
        </div>
      </div>
